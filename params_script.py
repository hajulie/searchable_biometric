...
def expected_number_root_matches(l, k, lsh_tpr, lsh_fpr, bf_fpr):
    tp_matches = k * lsh_tpr
    fp_matches = k * lsh_fpr    # ignoring BF FPR for now
...
def sys_params_to_csv(l, b,  n, lsh_tpr, lsh_fpr, bf_fpr, desired_tpr, desired_fpr, desired_max_k):
    ...
    with open(filename, 'w+') as f:
        ...
        for s in range(10, 100, 1):
            ...
            for k in range(1, desired_max_k, 100):
                ...
                if sys_tpr > desired_tpr and sys_fpr < desired_fpr:
                    ...
                    root_matches_with_ea, root_matches_wo_ea = expected_number_root_matches(l, k, lsh_tpr_s, lsh_fpr_s, bf_fpr)
                    # print("# root matches with early abort = " + str(root_matches_with_ea))
                    # print("# root matches without early abort = " + str(root_matches_with_ea))
            ...
            if min_trees < desired_max_k and max_trees > 0:
                row = ("Overall", s, min_trees, round(tmp_min_tpr, 3), round(tmp_min_fpr, 6), max_trees,
                       round(tmp_max_tpr, 3), round(tmp_max_fpr, 4))
                writer.writerow(row)
                print(row)
...
if __name__ == '__main__':
    ...
    filename = sys_params_to_csv(db_size, branch_factor, n, lsh_tpr, lsh_fpr, bf_fpr, desired_tpr, desired_fpr, desired_max_k)
